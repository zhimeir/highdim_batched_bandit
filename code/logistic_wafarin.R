...
lm_res1 <- glm(-(Y != 0) + 1~., data, family = binomial)
beta1 <- coef(lm_res1)
lm_res2 <- glm(-(Y != 1) + 1~., data, family = "binomial")
beta2 <- coef(lm_res2)
lm_res3 <- glm(-(Y != 2) + 1~., data, family = "binomial")
beta3 <- coef(lm_res3)
...
out_file <- sprintf("../results/warfarin_logistic_seed%d.txt", seed)
